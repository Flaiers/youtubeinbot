...
async def start(message: types.Message) -> None:
	button_youtube = InlineKeyboardButton('Подписаться', url='t.me/joinchat/R1Ct6BRjHCkO_AYO')
	inline_youtube = InlineKeyboardMarkup(row_width=1).add(button_youtube)

	await message.answer(
		'Подпишись на наш канал — Скачать Видео Ютуб в котором мы постим '
		'обновления бота, выкладываем интересную '
		'информацию в сфере IT, а также устраиваем розыгрыши!\n\n'
		'Мы будем очень благодарны тебе 🥰',
		reply_markup=inline_youtube
	)
	
	await message.answer(
		'Перейди в Главное меню, чтобы Скачать видео\n'
...
async def message(message: types.Message) -> None:
	if message.text == '🏠 Главное меню':
		...
		await message.answer(
			'Отправь мне ссылку на видеоролик, также можешь вернуться Назад, нажав на кнопку',
		...
		await message.answer(
			'Ты зашёл в Личный кабинет, в разделе\n'
...
@dp.message_handler(state=User.choice)
async def lk(message: types.Message, state: FSMContext) -> None:
	lk = message.text
		...
		if choice == '💎 Мои кристаллы':
			...
			try:
				...
				if user_channel["status"] == 'left':
					...
					await message.answer(
						'Нажми на 👆, чтобы обменять 💎. Хочешь вернуться в Личный кабинет, нажми Назад',
						reply_markup=kb.reply_back
					)
			...
			await message.answer(
				'Подписавшись на наши каналы, ты получаешь кристаллы. '
			...
			await message.answer(
				'Наш канал — Скачать Видео Ютуб в котором мы постим обновления бота, выкладываем интересную'
				'информацию в сфере IT, а также устраиваем розыгрыши!',
...
@dp.message_handler(state=User.video)
					...
					try:
						...
						try:
							...
							try:
								...
								await message.answer(
									'Ты вводишь какую-то неправильную ссылку, отправь её мне снова, или вернись в Назад',
									reply_markup=kb.reply_back
								)

				try:
					await message.answer('Отправляю... Подождите!')
					links = video_link(url)
					url_720, url_360, url_image = links
					...
					await message.answer(
						'Вот и кнопки на скачивание видеоролика\n'
					...
					await message.answer(
						'Теперь ты можешь ещё раз скачать видеоролик, или вернуться в Главное меню',
...
async def fla_check(callback_query: types.CallbackQuery, state: FSMContext) -> None:
	...
	if user_channel["status"] == 'left':
		...
		await bot.send_message(
			...
			'Я проверил, ты не подписался. Подпишись чтобы получить кристаллы 😉',
			reply_markup=kb.reply_back
		)
		...
		await bot.send_message(
			callback_query.from_user.id,
			'Я проверил, ты подписался. Уже начислил тебе 100 кристаллов!\n\n'
...
async def sell(callback_query: types.CallbackQuery, state: FSMContext) -> None:
	await bot.send_message(
		callback_query.from_user.id,
		'Недостаточно 💎 для действия. Обмен возможет только от 199, в скором времени '
		'у нас расширится база каналов и ты получишь еще больше кристаллов\n\n'
		'Если отпишешься от канала, ты потеряешь все заработанные 💎. Нажимай Назад, чтобы вернуться в Личный кабинет',
		reply_markup=kb.reply_back
	)
